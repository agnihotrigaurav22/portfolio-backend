...
class Project(db.Model):
    ...
    tech_stack = db.Column(db.String(200)) # start separated by commas
...
class Skill(db.Model):
    ...
    level = db.Column(db.Integer) # 1-100 for progress bars
    category = db.Column(db.String(50)) # Frontend, Backend, Tools
...
def seed_db():
    ...
    user = UserProfile(
        ...
        about_text="Data Science undergraduate at IIT Madras with hands-on experience in machine learning, data analysis, and model evaluation. Seeking AI/ML internship opportunities to build, validate, and deploy data-driven models for real-world problem solving.",
    ...
    exp1 = Experience(
        ...
        description="Analyzed and processed large datasets, performing feature engineering and data visualization. Collaborated with cross-functional teams to design and implement RESTful APIs for scalable AI-driven applications."
    )
    exp2 = Experience(
        role="Teaching Assistant, C Programming Lab",
        company="IIT Madras",
        duration="Jun 2024 – Dec 2024",
        description="Assisted undergraduate students in debugging, optimizing, and improving C programs. Provided guidance on syntax, logical reasoning, and effective problem-solving techniques."
    )
    ...
    p2 = Project(
        ...
        description="AI-driven web app generating personalized workout/diet plans using Google Gemini API. Modular Flask backend.",
    ...
    db.session.commit()
    print("Database seeded with resume data.")

# --- Routes ---
@app.route('/')
def index():
    user = UserProfile.query.first()
    # If no DB, handle gracefully or show error (for development)
    if not user:
        user = UserProfile(name="Setup Required", tagline="Please seed the database", about_text="Run `flask seed-db`")
    
    projects = Project.query.limit(3).all()
    skills = Skill.query.all()
    experiences = Experience.query.all()
    educations = Education.query.all()
    return render_template('index.html', user=user, projects=projects, skills=skills, experiences=experiences, educations=educations)
